...
def estimate_gradient_same_data(model, X_noisy, t, h, initial_alpha, iters, lr, device):
    """Projected-gradient alpha estimation on one fixed noisy trajectory."""
...
def estimate_grid_same_data(model, X_noisy, t, h, alpha_grid, device):
    """Fine-grid estimation using the same noisy trajectory and same loss."""
    X = torch.tensor(X_noisy, dtype=torch.float32, device=device)
    tt = torch.tensor(t.reshape(-1, 1), dtype=torch.float32, device=device)
    target = X - X[0:1, :]
...
def main():
    parser = argparse.ArgumentParser(
        description="Paired off-grid test: projected gradient vs fine grid."
    )
    parser.add_argument(
        "--checkpoint",
        required=True,
    ...
    # Same candidate grid used by the fine-search comparison.
    ...
    for rep in range(args.repeats):
        # Generate ONE trajectory and ONE noise realization.
        # Both methods below receive exactly this same noisy trajectory.
        ...
        u_noisy = (
            ...
            + args.noise
            * rms
            * np.random.randn(*u_clean.shape)
        )

        X_noisy = u_noisy.T  # (Nt, state_dim)
    ...
    # Correct paired test: trial i in each array uses the same noisy trajectory.
    ...
    print(
        f"Proposed gradient: "
        f"MAE = {np.mean(gradient_errors):.6f} ± "
        f"{np.std(gradient_errors):.6f}"
    )
    print(
        f"Fine grid (Δα=0.001): "
        f"MAE = {np.mean(grid_errors):.6f} ± "
